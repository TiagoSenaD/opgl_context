...
void init_BGLC(Display** dpy, xcb_connection_t** xcb_connection){
  ...
  int x11_screen_idx = DefaultScreen(*dpy);
  ...
  int glx_nfbconfigs;
  GLXFBConfig* glx_fbconfigs = glXChooseFBConfig(*dpy, x11_screen_idx,glx_fbconfig_attrs, &glx_nfbconfigs); if(glx_nfbconfigs == 0) printf("FAIL");
  GLXFBConfig glx_fbconfig = glx_fbconfigs[0];
  XFree(glx_fbconfigs);
  int glx_visual_id; glXGetFBConfigAttrib(*dpy, glx_fbconfig, GLX_VISUAL_ID, &glx_visual_id);
  ...
  GLXContext glx_context = glXCreateContextAttribsARB(*dpy, glx_fbconfig, NULL, 1, glx_context_attrs);
  glXMakeContextCurrent(*dpy, xcb_window, xcb_window, glx_context);
}
...
//TODO: finalizar as conexoẽs glx_context, xcb_colormap, xcb_window
